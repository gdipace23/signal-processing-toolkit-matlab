function [signalTheta, angleVec, pointsPerRev, numRevs] = angularResamplingWithTacho(...
...
%
%   [signalTheta, angleVec, pointsPerRev, numRevs] =
...
%       triggerLevel  - Threshold level to detect rising edges
...
%       pointsPerRev  - Total number of interpolated points per revolution
%       numRevs       - Number of revolutions detected
    ...
    % Detect rising edges in the tacho signal (threshold crossing)
    ...
    samplesPerRev = floor(fs / (freqMean * pulsesPerRev));
    pointsPerRev = samplesPerRev * pulsesPerRev;
    deltaTheta = 2 * pi / pointsPerRev;
    totalPoints = samplesPerRev * numRevs;
    angleVec = (0:totalPoints - 1)' * deltaTheta;

    % Compute time instants corresponding to each angle
    timeAtAngle = zeros(totalPoints, 1);
    deltaThetaSmall = 2 * pi / (samplesPerRev * pulsesPerRev);
    offset = 0;
    ...
    for i = 1:numRevs
        ...
        for k = 0:samplesPerRev-1
            theta = k * deltaThetaSmall;
            timeAtAngle(offset + k + 1) = (1 / (2 * b2)) * (sqrt(4 * b2 * (theta - b0) + b1^2) - b1);
        end
        offset = offset + samplesPerRev;
    end
